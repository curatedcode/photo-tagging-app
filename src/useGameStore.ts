...
export const useGameStatus = create<GameState>((set) => ({
  ...
  setGameStatus: (status: string) =>
    set((state) => ({ gameStatus: (state.gameStatus = status) })),
}));
...
export const useGameTime = create<GameTime>((set) => ({
  gameTime: "00:00:00",
  setGameTime: (time) =>
    set((state) => ({ gameTime: (state.gameTime = time) })),
}));
...
export const useGameCharactersLeft = create<GameCharacters>((set) => ({
  ...
  setCharactersLeft: (left) =>
    set((state) => ({ charactersLeft: (state.charactersLeft = left) })),
}));
...
export const useGameCharacterLastFound = create<GameCharacterLastFound>(
  (set) => ({
    characterLastFound: "",
    setCharacterLastFound: (found) =>
      set((state) => ({
        characterLastFound: (state.characterLastFound = found),
      })),
  })
);
